...
#### generate build --prod
RUN ng build --configuration=production --output-hashing=none && ng run parasol-web:server:production --output-hashing=none
...
RUN pwd
RUN ls -a
CMD node ./dist/parasol-web/server/main.js